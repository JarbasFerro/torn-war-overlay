...
<ul>
  ...
  <li>A plain-language match verdict on every row: <code>EASY</code>, <code>GOOD</code>, <code>RISKY</code> or <code>AVOID</code>. Estimated with a Public key before you have fought someone: Torn's rank name decodes into a hard stat band, and public training stats (xanax, refills, drinks, activity, account age) place the player within it; shown with a <code>~</code> and a plausible range; calibrated by your real fights; replaced by real Fair Fight and your win/loss record after.</li>
...
<table>
  ...
  <tr><th>Purpose of use</th><td>Ranked-war target overlay on the faction page and attack page.</td></tr>
